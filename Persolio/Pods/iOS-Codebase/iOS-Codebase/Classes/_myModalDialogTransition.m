...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
	...
	if (self.isPresenting)
	{
		...
		myDialog.contentView.transform = CGAffineTransformMakeScale(0, 0);
		...
		[UIView animateWithDuration:animationDuration delay:0.0 usingSpringWithDamping:0.65 initialSpringVelocity:0 options:0 animations:^{
			...
			myDialog.contentView.transform = CGAffineTransformMakeScale(1, 1);
	...
	else //dismissal
	{
		...
		[UIView animateWithDuration:.3 animations:^{
			myDialog.contentView.transform = CGAffineTransformMakeScale(0.01, 0.01);
			myDialog.contentView.alpha = 0;
			myDialog.visualEffectView.alpha = 0;

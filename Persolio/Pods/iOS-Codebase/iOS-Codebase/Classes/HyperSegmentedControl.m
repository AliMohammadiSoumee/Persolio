...
-(void)setSelectedIndex:(NSUInteger)selectedIndex animated:(BOOL)animated
{
	...
	CGRect viewFrame = [itemDic[@"view"] frame];
...
-(void)reloadView
{
	...
	for (NSUInteger i = 0; i != numberOfItems; i++)
	{
		...
		NSDictionary* dic = @{@"index": @(i), @"view": aView, @"size": [NSValue valueWithCGSize:aViewSize], @"gest": tapGest, @"color": aViewColor};
...
-(void)setPreciseIndex:(float)preciseIndex
{
	...
	CGRect lowerItemFrame = [lowerItemDic[@"view"] frame];
	...
	CGRect upperItemFrame = [upperItemDic[@"view"] frame];
